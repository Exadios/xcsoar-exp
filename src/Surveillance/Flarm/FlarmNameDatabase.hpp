...
/**
 * A class used to maintain FLARM target records.
 */
class FlarmNameDatabase
  ...
  public:
    struct Record
      ...
      Record() = default;
      Record(TargetId _id, const TCHAR* _name) noexcept
        :id(_id), name(_name) {}
      };
  ...
  public:
    ...
    [[gnu::pure]]
    const TCHAR* Get(TargetId id) const noexcept;

    [[gnu::pure]]
    TargetId Get(const TCHAR* name) const noexcept;
    ...
    unsigned Get(const TCHAR* name,
                 TargetId *buffer, unsigned max) const noexcept;

    bool Set(TargetId id, const TCHAR *name) noexcept;

  protected:
    [[gnu::pure]]
    int Find(TargetId id) const noexcept;
