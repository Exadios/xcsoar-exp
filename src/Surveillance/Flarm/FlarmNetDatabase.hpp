...
class FlarmNetDatabase
  {
    typedef std::map<TargetId, FlarmNetRecord> RecordMap;
    RecordMap map;
  ...
  public:
     ...
     */
    [[gnu::pure]]
    const FlarmNetRecord *FindRecordById(TargetId id) const
      {
      auto i = map.find(id);
      return i != map.end()
             ? &i->second
    ...
    unsigned FindRecordsByCallSign(const TCHAR *cn,
                                   ...
                                   unsigned size) const;
    unsigned FindIdsByCallSign(const TCHAR *cn, TargetId array[],
                               unsigned size) const;
