...
struct AdsbList
  ...
  TrivialArray<AdsbTarget, MAX_COUNT> list;
   ...
   * this one.
   */
  constexpr void Complement(const AdsbList& add) noexcept
    {
    ...
    for (auto &traffic : add.list)
      ...
      if (FindTraffic(traffic.id) == nullptr) 
        {
        AdsbTarget * new_traffic = AllocateTraffic();
   ...
   */
  AdsbTarget* FindTraffic(TargetId id) noexcept
    {
   ...
   * @return the FLARM_TRAFFIC pointer, NULL if not found
   */
  const AdsbTarget* FindTraffic(TargetId id) const noexcept
    {
   ...
   * @return the FLARM_TRAFFIC pointer, NULL if not found
   */
  constexpr AdsbTarget* FindTraffic(const TCHAR* name) noexcept
    {
    for (auto &traffic : list)
   ...
   * @return the FLARM_TRAFFIC pointer, NULL if not found
   */
  constexpr const AdsbTarget* FindTraffic(const TCHAR* name) const noexcept
    {
    for (const auto &traffic : list)
      if (traffic.name.equals(name))
        return &traffic;
   ...
   */
  constexpr AdsbTarget* AllocateTraffic() noexcept
    {
    return list.full()
      ? NULL
      : &list.append();
   ...
   */
  constexpr const AdsbTarget* PreviousTraffic(const AdsbTarget* t) const noexcept 
    {
    return t > list.begin()
      ? t - 1
      : NULL;
   ...
   */
  constexpr const AdsbTarget* NextTraffic(const AdsbTarget* t) const noexcept
    {
    return t + 1 < list.end()
      ? t + 1
      : NULL;
   ...
   */
  constexpr const AdsbTarget* FirstTraffic() const noexcept
    {
   ...
   */
  constexpr const AdsbTarget* LastTraffic() const noexcept
    {
   ...
   */
  [[gnu::pure]]
  const AdsbTarget* FindMaximumAlert() const noexcept;

  constexpr unsigned TrafficIndex(const AdsbTarget* t) const noexcept
    {
    return t - list.begin();
    }
  };

//static_assert(std::is_trivial<AdsbList>::value, "type is not trivial");
