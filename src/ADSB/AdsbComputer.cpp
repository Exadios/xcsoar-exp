...
AdsbComputer::Process(AdsbData &adsb,
  ...
  for (auto &traffic : adsb.traffic.list)
    ...
    if (!traffic.HasName())
      ...
      if (basic.call_sign_valid)
        traffic.name = basic.call_sign;
      }
    ...
    traffic.altitude_available = basic.altitude_available;
    if (traffic.altitude_available)
      traffic.altitude = traffic.relative_altitude +
                         RoughAltitude(basic.altitude);
    ...
    if (traffic.climb_rate_avg30s_available)
      traffic.climb_rate_avg30s =
        adsb_calculations.Average30s(traffic.id, basic.time, traffic.altitude);

    // The following calculations are only relevant for targets
    // where information is missing
    if (traffic.track_received &&
        traffic.speed_received &&
        traffic.climb_rate_received)
      continue;
