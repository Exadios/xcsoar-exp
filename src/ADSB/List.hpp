...
public:
  ...
  void Expire(double clock)
    {
    this->modified.Expire(clock, 300);
    this->new_traffic.Expire(clock, 60);
   ...
   */
  AdsbTraffic *FindTraffic(IcaoId id)
    {
   ...
   * @return the ADSB_TRAFFIC pointer, NULL if not found
   */
  const AdsbTraffic *FindTraffic(IcaoId id) const
    {
  ...
  const AdsbTraffic *PreviousTraffic(const AdsbTraffic *t) const
    {
    return t > this->list.begin() ? t - 1 : NULL;
    }
  ...
  const AdsbTraffic *NextTraffic(const AdsbTraffic *t) const
    {
    return t + 1 < this->list.end() ? t + 1 : NULL;
    }
