...
//      this->flarm_status.available == flarm_status.available         &&
  ...
  this->fr.SetAngle(-heading);
  this->fir.SetAngle(heading);
  this->data = new_data;
 ...
 */
void
FlarmTrafficWindow::PaintRadarNoGo(Canvas &canvas) const
{
  if (small)
    return;
