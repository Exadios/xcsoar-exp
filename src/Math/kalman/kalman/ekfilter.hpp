 ...
 * equations). This example suggests a procedure to follow and 
 * shows how to use the library.  If you are not familiar with the %Kalman
 * filter, please read this article [02].
 *
 * \subsection step1 Step 1 : Find the mathematical model of the system
 * The first thing to do is to find out the state vector you want to 
 * estimate \f$ \vec x \f$ and
 * the inputs \f$ \vec u \f$ of the system. After this, find the non-linear
 * process function \f$ f \f$ 
 * that describes the evolution of the state vector through time, that is :
 * \f[ \vec x_k = f \left( x_{k-1}, u_{k-1}, w_{k-1} \right) \f]
 * where \f$ w \f$ is the process noise vector due to uncertainty and process
 * modeling errors.
 * 
 *	Then, find the non-linear relation between your state vector
 * \f$ \vec x \f$ and the measure vector \f$ \vec z \f$.
 ...
 * \end{array} \right ] \f]
 *
 * where \f$ w_{1} \f$ and \f$ w_{2} \f$ are the random variables which
 * represent the process noise.
 *
 * A station on the ground (at the origin) mesures the angle between the
 * plane and the ground (x axis) and the distance between the plane and the
 * station.
 ...
 *
 * where \f$ v_{1} \f$ and \f$ v_{2} \f$ are the random variables which
 * represent the process noise.
 *
 ...
 *
 * Set initial estimation of the state vector. After, set the covariance
 * matrix P 
 ...
 * \par In this example:
 *
 * The first estimation of the state vector is based on the first measures
 ...
 * You should not try to optimize it at this step, just create your filter
 * and validate it.
 * This library allows you to optimize your filter, but in your first
 * implementation, 
 * code only the basic functions. So, code functions called
 * \c makeProcess(), \c makeMeasure(), 
 * \c makeA(), \c makeH(), \c makeQ(), \c makeR(), \c makeV() and \c makeW().
 ...
 * \until }
 *
 * In this example, our %Kalman filter inherits from the
 * Extended %Kalman Filter, because it's a non-linear problem (\f$ f \f$ and
 * \f$ h \f$ are non-linear functions)
 * The first two template parameters are respectively the floating point
 * type used by the filter (\c float or \c double) and the beginning index
 * of vectors and matrices (0 or 1). There are three other template parameters
 * to the \c EKFilter template class. 
 ...
 * 
 * You should declare each functions named previously in this class. You can
 * declare variables too.
 * 
 * After, code the class constructor. You can call the function \c setDim()
 * here or you will call it manually
 * in your \c main() function after you created the filter object.  The
 * function \c setDim() sets the number of states,
 * the number of inputs, the number of process noise random variables, the number of measures and the 
 * number of measurement noise random variables. It can be used by advanced
 * users to implement a
 * Variable-Dimension Extended %Kalman Filter (an EKF whose dimensions may
 * change from one iteration to the other).
 *
 ...
 * \until }
 *
 * In the function \c makeMeasure(), you update directly the measures vector
 * \f$ \vec z \f$.  These are the predicted measures.
 *
 ...
 * \until }
 *
 * Now, your filter is ready to be used.  In this example, the measures and
 * the inputs
 * have been calculated by the \c generation.m Matlab script. It's a good 
 * idea to test 
 ...
 * function before calling the \c init() function.
 * In this example, the \c setDim() function is called in the class
 * constructor.  The \c init() function 
 * sets the initial state and the initial covariance matrix. 
 *
 * WARNING : The vectors passed to the \c init() function become unusable when
 * \c init() returns ! Never use those vectors after the call.
 *
 ...
 *
 * When your %Kalman filter works properly, you can optimize it in many simple
 * ways.
 * 
 * -# If matrix Q is always diagonal, set the OQ template parameter to \c true.
 * This will
 * minimize some calculations. Also, you will only need to fill in diagonal
 * elements of Q,
 * since the other values will never be read.
 * -# If both matrices V and R are always diagonal, set the OVR template
 * parameter to \c true. This will
 * minimize some calculations. Also, you will only need to fill in diagonal
 * elements of V and R,
 * since the other values will never be read.
 * -# If some matrix values are constant, use the \c makeBaseX() function 
 * instead of the \c makeX() function
 * to fill these values.
 * These functions are called only once at the beginning instead of once per
 * iteration. You can used 
 * the \c makeBaseX() function to set values that never change in a matrix
 * and just set the other
 * values in the \c makeX() function.
 * -# If complex calculations are needed for more than one of \c makeA(),
 * \c makeW(), \c makeQ() and \c makeProcess()
 * functions, then use the function \c makeCommonProcess() to do those
 * calculations and save them in member variables of your own subclass.
 * This function is
 * always called before the others.
 * -# If complex calculations are needed for more than one of \c makeH(),
 * \c makeV(), \c makeR(), \c makeMeasure()
 * and \c makeDZ() functions, then use the function \c makeCommonMeasure()
 * to do those calculations and save them in member variables of your own 
 * subclass.  This function is
 * always called before the others.
 * -# While writing a \c makeX() or a \c makeBaseX() function, there can
 * be some execution paths where the function
 * does not modify any matrix (for example, if there is some condition, then
 * modify the matrix, else don't).
 * If this is the case, then each non-mutating execution path should call
 * \c NoModification() before returning,
 * so that some calculations can be avoided.
 * -# When your filter works properly, set the debug template parameter to
 * \c false to disable bound-checking.
 *
 * \par In this example:
 *
 * This example have been optimized in many ways:
 *
 * - The matrices V and R are diagonal, so the OVR template parameter is set
 * to true.  Then, we just have to 
 * set the diagonal values of those matrices.
 * - The matrices V, R, W and Q never change, so we use \c makeBaseX()
 * functions instead of \c makeX() functions.
 * - Constants values of A and H have been moved from the \c makeX()
 * functions to \c makeBaseX() functions.
 * - The filter works properly, so we don't need bound-checking. Let's set
 * the debug template parameter to \c false.
 *
