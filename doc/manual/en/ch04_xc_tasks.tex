...
\bmenu{Nav 2}\blink\bmenu{Task Resume} resumes the ordered task (if
any).
...
\begin{itemize}
\item
From the task manager, menu \bmenu{Nav 1}\blink\bmenu{Task} \todonum{descr. tabular "Turn Points"}
and highligh a waypoint, then tap the highlighted waypoint again to display the
Task point dialogue, then press the \bmenu{Details} \InfoBox.
...
\item
From the menu \bmenu{Nav 1}\blink\bmenu{Alternates} to show the
details of the landables nearest to the aircraft.

\item
From the waypoint selector, menu \bmenu{Nav 1}\blink\bmenu{Waypoint
...
\button{GoTo} to directly initiate
...
\item[Alt diff MC safety] Altitude difference at the abort/safety MacCready
  setting (see \ref{sec:safty-factors})
...
This may be accessed several ways:
\begin{itemize}
\item  From the menu \bmenu{Nav 1}\blink\bmenu{Waypoint List}
\item  From the task editor, menu \bmenu{Nav 1}\blink\bmenu{Task}
...
are buttons for \button{Calculator}, \button{Turn Points}, \button{Manage},
and \button{Rules}, as well as a button to \button{Close} the task manager.
...
\end{quote}
\item From the analysis dialogue, menu \bmenu{Info}\blink\bmenu{Analysis} and select
 the button \button{Task Calc}
\end{itemize}

\begin{center}
\includegraphics[angle=0,width=0.8\linewidth,keepaspectratio='true']{en/figures/dialog-taskcalculator.png}
\end{center}
...
The \button{Turn Points} button displays an ordered list of the points in the
...
\subsection*{Manage}
Tapping on the \button{Manage} button displays the task manager's management
screen.  There are four options, each of which is accessed by tapping on the
relevant button:

\begin{itemize}
\item \button{New Task} Clears the current task and resets the task rules to
  the default values.
\item \button{Declare}  If an external logger is connected, this will allow
  uploading the  active task to the logger and declaring it.
\item \button{Browse} Displays a list of all the saved tasks, allowing the
  pilot to load a previously saved task.  Note that this option will overwrite
  the current active task.
\item \button{Save}  Saves the current active task.  Upon tapping the
  \button{Save} button the pilot will be prompted to enter a file name for the
  task to be saved.
\end{itemize}

\subsection*{Rules}
The values in the \button{Rules} menu depend on the task type selected.  Clicking
...
Also, tapping on the \button{Rules} button again after it is highlighted allows
...
This is defined by tapping the \button{Turn Points} button from the Task Manager.
This brings up the list of waypoints in the task (if any).  Highlighting any waypoint
on the list and either tapping it again or pressing the \button{Edit Point} button
brings up the waypoint definition pressing the \button{Change type} button  will
...
\begin{itemize}
\item Pressing \bmenu{Nav 2}\blink\bmenu{Task Abort}
\item Choosing an alternate from the list under  \gesture{Down - Left}
 \bmenu{Nav 1}\blink\bmenu{Alternates}
...
\begin{quote}
\bmenu{Config 3}\blink\bmenut{Logger}{Start}
\end{quote}
...
menu:
\begin{quote}
\bmenu{Config 2}\blink\bmenu{Replay}
\end{quote}
...
\begin{quote}
\bmenu{Config 3}\blink\bmenu{Raw Logger}
\end{quote}
...
\begin{quote}
\bmenu{Info 1}\blink\bmenu{Analysis}
\end{quote}
