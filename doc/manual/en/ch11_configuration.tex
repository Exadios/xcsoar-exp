...
\menulabel{\bmenu{Config 2}\blink\bmenut{Setup}{System}}
...
\item[Final glide bar MC0*]  If set to ON the final glide bar will show a second arrow
  indicating the required height to reach the final waypoint at MC zero.
\end{description}
...
\item[Tracking Interval]  The time intervall for which one position is send to
  the tracking service. `{\em SkyLines}' implements a very lean protocoll, so that even
...
\item[Server]  The service's web address to stream your position to. Ther is currently
